...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function CheckBlockAcceptedInteral(blockHash, daemon, callback){
        daemon.cmd('getblock',
            [blockHash],
            function(results){
    ...
    function CheckBlockAccepted(blockHash, daemon, callback){
        if(options.hasLegacyDaemon) {
            setTimeout(function(){ CheckBlockAcceptedInteral(blockHash, daemon, callback); }, 3000);
        } else {
            CheckBlockAcceptedInteral(blockHash, daemon, callback);
        }
    }
